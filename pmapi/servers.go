		...
		if pmapi.ApiErr != nil {
			...
			if pmapi.ApiErr != nil {
				...
				if pmapi.ApiErr = c.Bind(&tmpserver); pmapi.ApiErr != nil {
					...
					delsrv, _ := proxysql.NewServer(tmpserver.HostGroupId, tmpserver.HostName, tmpserver.Port)
					pmapi.ApiErr = delsrv.DeleteOneServers(pmapi.Apidb)
					if pmapi.ApiErr != nil {
						switch {
						...
						c.JSON(http.StatusOK, gin.H{"exit": "0", "messages": strconv.Itoa(int(tmpserver.HostGroupId)) + "-" + tmpserver.HostName + "-" + strconv.Itoa(int(tmpserver.Port)) + " Delete Successed!"})
					}
	...
	if hostname == "" || hostname == "undefined" || port == "" || port == "undefined" || username == "" || username == "undefined" || password == "" || password == "undefined" {
		...
		if pmapi.ApiErr != nil {
			...
			if pmapi.ApiErr != nil {
				...
				delsrv, _ := proxysql.NewServer(tmpserver.HostGroupId, tmpserver.HostName, tmpserver.Port)
				pmapi.ApiErr = delsrv.UpdateOneServerInfo(pmapi.Apidb)
				if pmapi.ApiErr != nil {
					switch {
					case errors.IsNotFound(pmapi.ApiErr):
					...
					c.JSON(http.StatusOK, gin.H{"exit": "0", "messages": strconv.Itoa(int(tmpserver.HostGroupId)) + "-" + tmpserver.HostName + "-" + strconv.Itoa(int(tmpserver.Port)) + " Update Successed!"})
				}
			}
		}
	}
